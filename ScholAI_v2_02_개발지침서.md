...
async def search_nanet(keyword: str) -> list:
    ...
    params = {
        ...
        "startNo": 1,
        "displayCount": 20,
        "mediaCode": "BO",   # 도서 및 논문
    }
    async with httpx.AsyncClient() as client:
        response = await client.get(NANET_BASE, params=params, timeout=10.0)
        data = response.json()
        return [_parse_nanet_result(r) for r in data.get("result", {}).get("list", [])]
...
def _parse_nanet_result(item: dict) -> dict:
    return {
        ...
        "citation_count": 0,  # 국회도서관은 피인용 미제공
...
def import_kci_csv(csv_path: str, db_url: str):
    ...
    # 필수 컬럼 없으면 기본값
    for col in ["citation_count", "kci_grade", "doi", "abstract", "url"]:
...
*문서 버전: v2.0 | 작성일: 2025*
*v1 대비 변경: 전문가 패널 4인 · Scholar/국회도서관/KCI CSV 연동 · R/Python(Tableau 제거) · statistics_interpretations 테이블 추가*
